...
class SmartCosineQuboSolver(q.GeneralQuboSolver):
    """
    ...
    def __cosine_between_2_variables(self,x1,x2,y1,y2):
        """
        Computes the cosine similarity between 2 vectors according
        ...
        Returns:
        int: The 'cosine similarity' between the 2 variables
        """
    ...
    def build_bqm(self, batch, percentage):
        ...
        for i, _ in enumerate(batch.data):
            for j, _ in enumerate(batch.data):
                if (i != j):
                    qubo[(i, j)] = self.__cosine_between_2_variables(
